...
export const visitor = (doorIndex: number, doors: Door[]): Door[] =>
...
export const printer = (doors: Door[]) =>
  doors.map((door) => (door.isOpen ? "@" : "#")).join("");

export const run = (): string => {
  const doors = init();
  const visits = doors.length;
  const afterVisit = visitAll(visits, doors);
  return printer(afterVisit);
};
